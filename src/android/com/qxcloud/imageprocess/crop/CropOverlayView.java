...
public class CropOverlayView extends View {
    ...
    public void setAspectRatioX(int aspectRatioX) {
        if (aspectRatioX <= 0)
            ...
            Log.e(TAG, "setAspectRatioX: 获得宽度 == " + aspectRatioX);
            mAspectRatioX = aspectRatioX;
            mTargetAspectRatio = ((float) mAspectRatioX) / mAspectRatioY;
            Log.e(TAG, "mTargetAspectRatio 1: 宽高比 == " + mTargetAspectRatio);
            if (initializedCropWindow) {
                Log.e(TAG, "initializedCropWindow: == "+initializedCropWindow);
                initCropWindow(mBitmapRect);
                invalidate();
            }
        }
    ...
    public void setAspectRatioY(int aspectRatioY) {
        Log.e(TAG, "setAspectRatioY: 获得高度 == " + aspectRatioY);
        if (aspectRatioY <= 0)
            throw new IllegalArgumentException(
                    "Cannot set aspect ratio value to a number less than or equal to 0.");
        else {
            ...
            mTargetAspectRatio = ((float) mAspectRatioX) / mAspectRatioY;
            Log.e(TAG, "mTargetAspectRatio 2: 宽高比 == " + mTargetAspectRatio);
            if (initializedCropWindow) {
                initCropWindow(mBitmapRect);
                invalidate();
            }
    ...
    private void initCropWindow(Rect bitmapRect) {
        ...
        if (mFixAspectRatio) {
            Log.e(TAG, "mFixAspectRatio : ==" + mFixAspectRatio);
            ...
            if (AspectRatioUtil.calculateAspectRatio(bitmapRect) > mTargetAspectRatio) {
                Log.e(TAG, "AspectRatioUtil.calculateAspectRatio(bitmapRect): ==" + AspectRatioUtil.calculateAspectRatio(bitmapRect) );
                Log.e(TAG, "mTargetAspectRatio: ==" + mTargetAspectRatio );
